-- ═══════════════════════════════════════════════════════════
-- 🆕 STEP 2: REBUILD — run this AFTER the SuperNuke (00a)
-- ═══════════════════════════════════════════════════════════
-- Paste into Command Bar (EDIT MODE) → Enter
-- This creates everything fresh. Run 00a (SuperNuke) FIRST.
-- ═══════════════════════════════════════════════════════════
...
-- 1) REMOTES (includes NEW AdminStatusEvent for reliable admin detection)
...
create("RemoteEvent",    "AdminStatusEvent", remotes)  -- 🆕 server → client: "you are admin"
...
if not dsOk then warn("⚠️ DataStore not available! Game Settings → Security → Enable Studio Access to API Services") end
...
local function saveData(player)
	...
	if not success then warn("Save failed for " .. player.Name) end
end
...
-- 🆕 SERVER TELLS THE CLIENT IF THEY'RE ADMIN (no hanging InvokeServer!)
Players.PlayerAdded:Connect(function(player)
	loadData(player)
	task.wait(1)
	if isAdmin(player) then
		AdminStatusEvent:FireClient(player, true)
		print("👑 " .. player.Name .. " is an admin — AdminUI activated.")
	end
end)
...
-- 7) ADMIN UI  (🆕 COMPLETELY REWORKED: listens for server event, no hanging!)
...
-- 🆕 LISTEN for the server to tell us we're admin (no InvokeServer that can hang!)
-- The server fires AdminStatusEvent to the player after they join + load.
...
-- 🆕 when the server confirms we're admin, build the UI
AdminStatusEvent.OnClientEvent:Connect(function(isAdmin)
	if isAdmin then
		buildAdminUI()
	end
end)
print("⏳ AdminUI waiting for server confirmation...")
]==]

print("══════════════════════════════════════")
print("✅ REBUILD COMPLETE! v7")
print("Scripts: AuraData, GameServer, RollUI, InventoryUI, StatsUI, AdminUI")
print("🆕 Admin now uses event-based detection (no hanging!)")
print("══════════════════════════════════════")
print("➡️ Press Play to test!")
