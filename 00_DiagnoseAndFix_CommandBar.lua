-- ═══════════════════════════════════════════════════════════
-- 🔍 DIAGNOSE + 💣 NUKE + 🆕 REBUILD  (ONE paste, run in EDIT mode)
-- ═══════════════════════════════════════════════════════════
-- This script FIRST prints what exists, THEN nukes, THEN rebuilds.
-- Watch the output — it tells us exactly what's wrong.
-- ═══════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════
-- 🔍 PART 1: DIAGNOSE — print everything we find
-- ═══════════════════════════════════════════════════════════
print("══════════════════════════════════════")
...
local rngNames = {
	AuraData=true, AuraDatabase=true, RNGManager=true, RollServer=true, GameServer=true,
	RollClient=true, RollUI=true, InventoryUI=true, StatsUI=true, AdminPanel=true, AdminUI=true,
	RollGui=true, InventoryGui=true, StatsGui=true, AdminPanelGui=true,
	Remotes=true, RollFunction=true, AnnounceEvent=true, GetInventoryFunction=true,
	EquipFunction=true, AdminFunction=true, GetStatsFunction=true, StatsUpdatedEvent=true,
	AdminStatusEvent=true,
}

local found = 0
local function scan(parent, path)
	for _, child in ipairs(parent:GetChildren()) do
		if rngNames[child.Name] then
			local dup = ""
			-- check for duplicates
			local count = 0
			for _, c in ipairs(parent:GetChildren()) do
				if c.Name == child.Name then count = count + 1 end
			end
			if count > 1 then dup = " ⚠️ DUPLICATE (" .. count .. " copies!)" end
			print("  📌 " .. path .. "/" .. child.Name .. " (" .. child.ClassName .. ")" .. dup)
			found = found + 1
		end
...
scan(SSS, "ServerScriptService")
scan(SPS, "StarterPlayerScripts")
scan(SG,  "StarterGui")
scan(SP,  "StarterPlayer")

-- also scan StarterPlayer itself (not just Scripts) for loose LocalScripts
for _, child in ipairs(SP:GetChildren()) do
	if child:IsA("LocalScript") then
		print("  📌 StarterPlayer/" .. child.Name .. " (LocalScript) ⚠️ LOOSE SCRIPT IN WRONG PLACE!")
		found = found + 1
	end
end

if found == 0 then print("  (nothing rng-related found — clean)") end
print("══════════════════════════════════════")

-- ═══════════════════════════════════════════════════════════
-- 💣 PART 2: NUKE — destroy everything recursively
-- ═══════════════════════════════════════════════════════════
local function deepNuke(parent)
	for _, child in ipairs(parent:GetChildren()) do
		deepNuke(child)
		if rngNames[child.Name] then
			child:Destroy()
...
-- ═══════════════════════════════════════════════════════════
-- 🆕 PART 3: REBUILD (with self-cleaning GUIs + robust admin)
-- ═══════════════════════════════════════════════════════════
...
create("RemoteEvent",    "AdminStatusEvent", remotes)
...
-- GAME SERVER (with case-insensitive admin + diagnostic prints)
...
local ADMIN_USERNAMES  = { "Twix79i" }   -- case-insensitive now!
...
-- 🔍 CASE-INSENSITIVE admin check (fixes username casing issues!)
...
RollFunction.OnServerInvoke = function(player)
	...
	if aura.Rarity > data.RarestRarity then data.RarestRarity = aura.Rarity; data.RarestAura = aura.Name end
...
AdminFunction.OnServerInvoke = function(player, action, value)
	...
	elseif action == "GiveRare" then
		local aura = AuraData.GetWeightedRandom(5000); table.insert(data.Inventory, aura.Name); StatsUpdatedEvent:FireClient(player, buildStats(data)); return aura.Name
...
-- 🔍 DIAGNOSTIC: print the exact username so we can verify admin matching
Players.PlayerAdded:Connect(function(player)
	print("👤 Player joined: Name='" .. player.Name .. "' DisplayName='" .. player.DisplayName .. "' UserId=" .. player.UserId)
	loadData(player)
	task.wait(1.5)
	local admin = isAdmin(player)
	print("🔍 isAdmin check for '" .. player.Name .. "': " .. tostring(admin))
	if admin then
		AdminStatusEvent:FireClient(player, true)
		print("👑 " .. player.Name .. " IS admin — AdminStatusEvent fired!")
	else
		print("❌ " .. player.Name .. " is NOT admin. Checking against: Twix79i")
	end
end)
...
print("✅ GameServer running! Admin usernames: " .. table.concat(ADMIN_USERNAMES, ", "))
]==]

-- ROLL UI (with SELF-CLEAN: destroys ALL old RollGui before creating!)
...
-- 🧹 SELF-CLEAN: destroy ALL existing RollGui (handles duplicates!)
...
banner.BackgroundColor3 = THEME_COLOR; banner.TextColor3 = BANNER_COLOR
...
button.MouseButton1Click:Connect(function()
	...
	if res.Rarity >= SHAKE_THRESHOLD then setGlow(true); shakeLabel(result, RESULT_HOME, res.Rarity) end
	isRolling = false
end)
...
-- 🧹 SELF-CLEAN
for _, child in ipairs(playerGui:GetChildren()) do
	if child.Name == "InventoryGui" then child:Destroy() end
end
...
local function refresh()
	...
	table.sort(names, function(a,b) return (AuraData.GetByName(a) and AuraData.GetByName(a).Rarity or 0) > (AuraData.GetByName(b) and AuraData.GetByName(b).Rarity or 0) end)
	for _, name in ipairs(names) do
		local aura = AuraData.GetByName(name); local count = data.Counts[name]
		local item = Instance.new("TextButton")
		item.Text = name .. "\n(×" .. count .. ")"; item.Font = Enum.Font.GothamBold; item.TextScaled = true
		item.BackgroundColor3 = ITEM_COLOR; item.TextColor3 = (aura and aura.Color) or TEXT_COLOR
		if name == equipped then
			item.BackgroundColor3 = EQUIPPED_COLOR; item.Text = "✓ " .. name .. "\n(×" .. count .. ")"
...
-- 🧹 SELF-CLEAN
for _, child in ipairs(playerGui:GetChildren()) do
	if child.Name == "StatsGui" then child:Destroy() end
end
...
panel.Size = UDim2.fromScale(0.20,0.20); panel.Position = UDim2.fromScale(0.78,0.02)
...
-- ADMIN UI (SELF-CLEAN + event-based, no hanging)
...
local function buildAdminUI()
	...
	local window = Instance.new("Frame")
	window.Size = UDim2.fromScale(0.35,0.78); window.Position = UDim2.fromScale(0.32,0.11)
	window.BackgroundColor3 = BG_COLOR; window.Visible = false
	local wCorner = Instance.new("UICorner"); wCorner.CornerRadius = UDim.new(0.04,0); wCorner.Parent = window
	...
	local title = Instance.new("TextLabel")
	title.Size = UDim2.fromScale(1,0.06); title.Position = UDim2.fromScale(0,0.02)
	title.Text = "🛠️  Admin Panel"; title.Font = Enum.Font.GothamBlack; title.TextScaled = true
	title.BackgroundTransparency = 1; title.TextColor3 = TEXT_COLOR; title.Parent = window

	local closeBtn = Instance.new("TextButton")
	closeBtn.Size = UDim2.fromScale(0.12,0.05); closeBtn.Position = UDim2.fromScale(0.85,0.015)
	closeBtn.Text = "✕"; closeBtn.Font = Enum.Font.GothamBold; closeBtn.TextScaled = true
	closeBtn.BackgroundColor3 = Color3.fromRGB(80,30,40); closeBtn.TextColor3 = TEXT_COLOR
	local cbCorner = Instance.new("UICorner"); cbCorner.CornerRadius = UDim.new(0.2,0); cbCorner.Parent = closeBtn; closeBtn.Parent = window

	local giveLabel = Instance.new("TextLabel")
	giveLabel.Size = UDim2.fromScale(0.9,0.04); giveLabel.Position = UDim2.fromScale(0.05,0.09)
	giveLabel.Text = "Give yourself an aura:"; giveLabel.Font = Enum.Font.GothamBold; giveLabel.TextScaled = true
	giveLabel.TextXAlignment = Enum.TextXAlignment.Left; giveLabel.BackgroundTransparency = 1; giveLabel.TextColor3 = TEXT_COLOR; giveLabel.Parent = window

	local scroll = Instance.new("ScrollingFrame")
	scroll.Size = UDim2.fromScale(0.9,0.48); scroll.Position = UDim2.fromScale(0.05,0.14)
	scroll.BackgroundTransparency = 1; scroll.ScrollBarThickness = 5
	scroll.CanvasSize = UDim2.fromScale(0,0); scroll.AutomaticCanvasSize = Enum.AutomaticSize.Y; scroll.Parent = window

	local list = Instance.new("UIListLayout"); list.Padding = UDim.new(0.01,0); list.Parent = scroll
	...
	for _, aura in ipairs(sorted) do
		local item = Instance.new("TextButton")
		item.Size = UDim2.fromScale(1,0.08); item.Text = aura.Name .. "  (1 in " .. aura.Rarity .. ")"
		item.Font = Enum.Font.GothamMedium; item.TextScaled = true; item.BackgroundColor3 = FIELD_COLOR; item.TextColor3 = aura.Color
		local iCorner = Instance.new("UICorner"); iCorner.CornerRadius = UDim.new(0.15,0); iCorner.Parent = item; item.Parent = scroll
	...
	luckLabel.Size = UDim2.fromScale(0.9,0.04); luckLabel.Position = UDim2.fromScale(0.05,0.64)
	luckLabel.Text = "Luck multiplier:"; luckLabel.Font = Enum.Font.GothamBold; luckLabel.TextScaled = true
	luckLabel.TextXAlignment = Enum.TextXAlignment.Left; luckLabel.BackgroundTransparency = 1; luckLabel.TextColor3 = TEXT_COLOR; luckLabel.Parent = window

	local luckBox = Instance.new("TextBox")
	luckBox.Size = UDim2.fromScale(0.55,0.05); luckBox.Position = UDim2.fromScale(0.05,0.69)
	luckBox.Text = "1"; luckBox.Font = Enum.Font.GothamMedium; luckBox.TextScaled = true
	luckBox.BackgroundColor3 = FIELD_COLOR; luckBox.TextColor3 = TEXT_COLOR; luckBox.ClearTextOnFocus = false; luckBox.Numeric = true
	local lbCorner = Instance.new("UICorner"); lbCorner.CornerRadius = UDim.new(0.15,0); lbCorner.Parent = luckBox; luckBox.Parent = window

	local luckBtn = Instance.new("TextButton")
	luckBtn.Size = UDim2.fromScale(0.3,0.05); luckBtn.Position = UDim2.fromScale(0.64,0.69)
	luckBtn.Text = "Set Luck"; luckBtn.Font = Enum.Font.GothamBold; luckBtn.TextScaled = true
	luckBtn.BackgroundColor3 = BUTTON_COLOR; luckBtn.TextColor3 = TEXT_COLOR
	local lkbCorner = Instance.new("UICorner"); lkbCorner.CornerRadius = UDim.new(0.15,0); lkbCorner.Parent = luckBtn; luckBtn.Parent = window
	...
	rareBtn.Size = UDim2.fromScale(0.43,0.06); rareBtn.Position = UDim2.fromScale(0.05,0.77)
	rareBtn.Text = "🎲 Give Rare"; rareBtn.Font = Enum.Font.GothamBold; rareBtn.TextScaled = true
	rareBtn.BackgroundColor3 = Color3.fromRGB(150,80,200); rareBtn.TextColor3 = TEXT_COLOR
	local rbCorner = Instance.new("UICorner"); rbCorner.CornerRadius = UDim.new(0.15,0); rbCorner.Parent = rareBtn; rareBtn.Parent = window
	...
	clearBtn.Size = UDim2.fromScale(0.43,0.06); clearBtn.Position = UDim2.fromScale(0.52,0.77)
	clearBtn.Text = "🗑️ Clear All"; clearBtn.Font = Enum.Font.GothamBold; clearBtn.TextScaled = true
	clearBtn.BackgroundColor3 = Color3.fromRGB(200,60,60); clearBtn.TextColor3 = TEXT_COLOR
	local clbCorner = Instance.new("UICorner"); clbCorner.CornerRadius = UDim.new(0.15,0); clbCorner.Parent = clearBtn; clearBtn.Parent = window
	clearBtn.MouseButton1Click:Connect(function()
		AdminFunction:InvokeServer("ClearInventory"); clearBtn.Text = "✓ Cleared"; task.wait(0.8); clearBtn.Text = "🗑️ Clear All"
	end)

	local resetBtn = Instance.new("TextButton")
	resetBtn.Size = UDim2.fromScale(0.9,0.06); resetBtn.Position = UDim2.fromScale(0.05,0.85)
	resetBtn.Text = "♻️ Reset ALL Data"; resetBtn.Font = Enum.Font.GothamBold; resetBtn.TextScaled = true
	resetBtn.BackgroundColor3 = Color3.fromRGB(180,40,40); resetBtn.TextColor3 = TEXT_COLOR
	local rsbCorner = Instance.new("UICorner"); rsbCorner.CornerRadius = UDim.new(0.15,0); rsbCorner.Parent = resetBtn; resetBtn.Parent = window
	resetBtn.MouseButton1Click:Connect(function()
		AdminFunction:InvokeServer("ResetData"); resetBtn.Text = "✓ Data Reset"; task.wait(0.8); resetBtn.Text = "♻️ Reset ALL Data"
	end)

	local isOpen = false
	openBtn.MouseButton1Click:Connect(function() isOpen = not isOpen; window.Visible = isOpen end)
	closeBtn.MouseButton1Click:Connect(function() isOpen = false; window.Visible = false end)
	print("🛠️ AdminUI built!")
end
...
print("══════════════════════════════════════")
print("✅ DIAGNOSE + NUKE + REBUILD COMPLETE!")
print("══════════════════════════════════════")
print("➡️ Press Play, then check the OUTPUT for:")
print("   '👤 Player joined: Name=...'  (see your exact username)")
print("   '👑 ... IS admin'  or  '❌ ... is NOT admin'")
print("📋 Copy those 2 lines and send them to me!")
