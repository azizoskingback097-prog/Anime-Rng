-- ═══════════════════════════════════════════════════════════
-- 💬 DIALOGUE SYSTEM (Final Fix + Easy Customization!)
-- ═══════════════════════════════════════════════════════════
...
if not remotes:FindFirstChild("DialogueEvent") then Instance.new("RemoteEvent", remotes).Name = "DialogueEvent" end
if not remotes:FindFirstChild("ShopOpenEvent") then Instance.new("RemoteEvent", remotes).Name = "ShopOpenEvent" end


-- ═══ 1. INSTALL SERVER SCRIPT ═══
...
-- 🗣️ DIALOGUE TEXTS (Change what he says here!)
...
-- 📋 THE OPTIONS (Change the text the player sees here!)
local CHOICES = {
...
DialogueEvent.OnServerEvent:Connect(function(player, action)
	...
	elseif action == "Story" then
		showText(STORY_RESPONSES[math.random(1, #STORY_RESPONSES)], 7)
		task.delay(7, function()
			if player and player.Parent then
				showText(GREETINGS[math.random(1, #GREETINGS)], 5)
				DialogueEvent:FireClient(player, { Type = "ShowChoices", NpcText = greeting, Choices = CHOICES })
			end
		end)
	elseif action == "Nevermind" then
		showText(NEVERMIND_RESPONSES[math.random(1, #NEVERMIND_RESPONSES)], 3)
	end
end)
]==]


-- ═══ 2. INSTALL CLIENT SCRIPT (With easy config!) ═══
...
-- ⚙️ CUSTOMIZATION CONFIG (Change UI here!)
local CONFIG = {
	TextSizeX = 280,           -- Width of the text/buttons
	TextSizeY = 35,            -- Height of the text/buttons
	Font = Enum.Font.FredokaOne, -- The font style
	TextColor = Color3.fromRGB(255, 255, 255), -- Normal text color (White)
	HoverColor = Color3.fromRGB(255, 235, 100), -- Hover color (Yellow)
	StrokeColor = Color3.fromRGB(0, 0, 0), -- Outline color (Black)
}
...
container.Position = UDim2.fromScale(0.98, 0.5) -- Right side of screen
...
local npcScale = Instance.new("UIScale", npcText)
npcScale.Scale = 0
...
print("✅ DIALOGUE SYSTEM FIXED & CONFIGURABLE!")
print("⚙️ To customize: Open DialogueUI in StarterPlayerScripts and edit the CONFIG table!")
