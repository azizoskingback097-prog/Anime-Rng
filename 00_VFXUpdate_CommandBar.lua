-- ═══════════════════════════════════════════════════════════
-- ✨ VFX SYSTEM DEPLOY — adds VFXData + VFXClient + updates GameServer
-- ═══════════════════════════════════════════════════════════
-- HOW TO USE: View > Command Bar → paste ALL of this → Enter
--
-- Creates:
--   • VFXData (ModuleScript in ReplicatedStorage)
--   • VFXClient (LocalScript in StarterPlayerScripts)
--   • EquippedChangedEvent (new remote)
--   • Updates GameServer to fire equip events
-- ═══════════════════════════════════════════════════════════
...
VFXData.VFX["Fire Burst"] = { Name = "Fire Burst", Effects = {
	{ Type = "Fire", Part = "HumanoidRootPart", Color = Color3.fromRGB(255,100,30), SecondaryColor = Color3.fromRGB(255,200,50), Size = 2.5, Heat = 15 },
	{ Type = "Particle", Part = "HumanoidRootPart",
...
VFXData.VFX["Crystal Aura"] = { Name = "Crystal Aura", Effects = {
	{ Type = "Particle", Part = "UpperTorso",
	  ...
	  LightEmission = 0.8, Rotation = NumberRange.new(0,360), RotSpeed = NumberRange.new(-180,180) },
...
VFXData.VFX["Shadow Flame"] = { Name = "Shadow Flame", Effects = {
	{ Type = "Fire", Part = "HumanoidRootPart", Color = Color3.fromRGB(80,0,120), SecondaryColor = Color3.fromRGB(40,0,60), Size = 3, Heat = 10 },
	{ Type = "Smoke", Part = "HumanoidRootPart", Color = Color3.fromRGB(40,0,60), Size = 2, Opacity = 0.5, RiseAcceleration = 2 },
	{ Type = "Particle", Part = "HumanoidRootPart",
...
VFXData.VFX["Divine Wind"] = { Name = "Divine Wind", Effects = {
	{ Type = "Particle", Part = "HumanoidRootPart",
	  ...
	  LightEmission = 1, Rotation = NumberRange.new(0,360), RotSpeed = NumberRange.new(180,360) },
	{ Type = "Particle", Part = "HumanoidRootPart",
...
local function createLight(part, cfg)
	...
	l:SetAttribute("TargetBrightness", cfg.Brightness or 1)
	l.Parent = part
	return l
end
...
local function applyVFX(character, vfxConfig)
	...
	for _, effect in ipairs(vfxConfig.Effects) do
		...
		if inst then table.insert(created, inst) end
	end
	task.spawn(function()
		task.wait(0.1)
...
local function updateVFX()
	...
	if vfxConfig then applyVFX(character, vfxConfig) end
end
...
print("VFXClient loaded!")
]==]

-- ═══ 4) GAME SERVER (updated — fires EquippedChangedEvent) ═══
...
print("══════════════════════════════════════")
print("✅ VFX SYSTEM DEPLOYED!")
print("══════════════════════════════════════")
print("✨ 7 VFX types: Smoke Mist, Stardust, Fire Burst,")
print("   Crystal Aura, Shadow Flame, Holy Burst, Divine Wind")
print("🎮 Test: equip an aura in Inventory → VFX appears!")
print("📖 Read VFX_GUIDE.md for full documentation!")
print("══════════════════════════════════════")
