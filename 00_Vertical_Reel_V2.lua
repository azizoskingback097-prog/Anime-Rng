-- ═══════════════════════════════════════════════════════════
-- 🎰 VERTICAL REEL V2 (Clean + Card Pop + Auto Hide)
-- ═══════════════════════════════════════════════════════════
-- Paste in Command Bar → Enter
--
-- FIXES:
--   • Reel + Center Line HIDE completely when not rolling
--   • Cards "pop" bigger as the center line crosses them
--   • Extra auras visible below the winner
--   • Reel fades out smoothly after stopping
--   • Clean small impact for good pulls
-- ═══════════════════════════════════════════════════════════
...
-- Center Line (Only visible during roll!)
...
local function createCard(auraData, index)
	...
	-- Store original size for pop effect
...
-- ═══════════════════════════════════════════════════════════
-- CARD POP EFFECT (When center line crosses!)
-- ═══════════════════════════════════════════════════════════
...
local function startCardPopTracker()
	...
	popConnection = RunService.RenderStepped:Connect(function()
		...
		-- Where is the center line in reel-local space?
		...
		if closestIndex ~= lastPoppedIndex and closestIndex >= 0 then
			...
			if lastPoppedIndex >= 0 then
				...
				if oldCard then
					...
					TweenService:Create(oldCard, TweenInfo.new(0.15), {
			...
			if newCard then
				...
				TweenService:Create(newCard, TweenInfo.new(0.1), {
					Size = UDim2.fromOffset(bw + 20, bh + 20)
				}):Play()
				
				-- Tick sound when crossing
...
local function playReel(res)
	...
	-- Generate 60 cards (winner at index 54, so 6 cards visible after it!)
	local totalCards = 60
	...
	for i = 0, totalCards - 1 do
		...
		if i == winnerIndex then
			cardData = res -- Actual Winner
		elseif isTeaseRoll and i == teaseIndex then
			cardData = rareAuras[math.random(1, #rareAuras)] -- Fake Rare!
		else
	...
	-- Start card pop tracker!
	startCardPopTracker()
	
	if isTeaseRoll then
		-- 1. FAKE STOP at tease card
	...
	-- Small UI Shake
	local shakeIntensity = res.Rarity >= 1000 and 5 or 2
	local t0 = os.clock()
	...
	-- Highlight the winning card (Make it glow!)
	...
	-- Hold the reel for 2 seconds so player sees the result + auras below
	task.wait(2)
	
	-- Fade out the reel completely
...
print("RollUI loaded! (Vertical Reel V2 - Card Pop + Auto Hide)")
]==]

print("✅ VERTICAL REEL V2 APPLIED!")
print("👁️ Reel + Line HIDE completely when idle.")
print("✨ Cards POP bigger as the line crosses them!")
print("⬇️ Extra auras visible below the winner.")
print("🔇 Reel fades out smoothly after stopping.")
