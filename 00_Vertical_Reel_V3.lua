-- ═══════════════════════════════════════════════════════════
-- 🎰 VERTICAL REEL V3 (Continuous Tease / Creep Suspense!)
-- ═══════════════════════════════════════════════════════════
-- Paste in Command Bar → Enter
--
-- THE NEW TEASE MECHANIC:
--   • No more "stop and go".
--   • The reel continuously moves, but slows down to a crawl 
--     near the rare aura to trick the player, then barely 
--     creeps past it to land on the actual result!
-- ═══════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════
-- MAIN VERTICAL REEL ANIMATION (With Continuous Creep Tease!)
-- ═══════════════════════════════════════════════════════════
...
local function playReel(res)
	...
	if isTeaseRoll then
		if isTeaseBefore then
			...
			-- 1. Fast spin to just before Rare
			local t1 = TweenService:Create(reel, TweenInfo.new(1.5, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
				Position = UDim2.fromOffset(0, getTargetY(teaseIndex - 1.5))
			})
			t1:Play()
			t1.Completed:Wait()
			
			-- 2. Creep slowly across Rare
			local t2 = TweenService:Create(reel, TweenInfo.new(1.2, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), {
				Position = UDim2.fromOffset(0, getTargetY(teaseIndex + 0.5))
			})
			t2:Play()
			t2.Completed:Wait()
			
			-- 3. Barely keep creeping to Actual
			local t3 = TweenService:Create(reel, TweenInfo.new(0.8, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
			...
			-- 1. Fast spin to just before Actual
			local t1 = TweenService:Create(reel, TweenInfo.new(1.5, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
				Position = UDim2.fromOffset(0, getTargetY(winnerIndex - 1.5))
			})
			t1:Play()
			t1.Completed:Wait()
			
			-- 2. Creep slowly over Actual, towards Rare (Stop at edge!)
			local t2 = TweenService:Create(reel, TweenInfo.new(1.5, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), {
				Position = UDim2.fromOffset(0, getTargetY(winnerIndex + 0.85)) -- 85% of the way to Rare
			})
			t2:Play()
			t2.Completed:Wait()
			
			-- 3. Settle back slightly to exact center
			task.wait(0.2) -- Pause at the edge for tension!
...
print("RollUI loaded! (Vertical Reel V3 - Continuous Tease)")
]==]

print("✅ VERTICAL REEL V3 APPLIED!")
print("🪝 The tease is now a continuous 'creep' that creates massive suspense!")
