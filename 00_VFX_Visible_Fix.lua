-- ═══════════════════════════════════════════════════════════
-- 👁️ VFX VISIBLE FIX — Makes the VFX show up but still NO pushing!
-- ═══════════════════════════════════════════════════════════
-- Paste in Command Bar → Enter
--
-- FIX: Previous code made everything invisible (Transparency = 1).
-- But VFX packs use visible glowing parts! Now we keep them VISIBLE.
--
-- How it prevents pushing WITHOUT making it invisible:
--   1. CanCollide = false (you walk through it)
--   2. Massless = true (it has no weight)
--   3. Anchored Tracker (follows you without physics)
-- ═══════════════════════════════════════════════════════════
...
print("VFX CLIENT LOADED! (Visible Fix)")
...
local function attachRig(character, rigName)
	...
	-- 1️⃣ PURGE pushing forces (the engines that push you!)
	...
	-- 2️⃣ Configure parts: No collision + no weight, but KEEP THEM VISIBLE!
	for _, obj in ipairs(clone:GetDescendants()) do
		if obj:IsA("BasePart") then
			-- DO NOT set Transparency = 1 (that hides the VFX!)
	...
	-- 3️⃣ CREATE THE TRACKER PART (invisible, anchored, follows player)
	...
	-- 4️⃣ WELD THE RIG TO THE TRACKER
	for _, obj in ipairs(clone:GetDescendants()) do
	...
	print("VFX: SUCCESS! '" .. rigName .. "' attached & VISIBLE! (" .. #parts .. " parts)")
	return true
end
...
print("✅ VFX VISIBLE FIX APPLIED!")
print("👁️ VFX is now VISIBLE but still has ZERO pushing!")
