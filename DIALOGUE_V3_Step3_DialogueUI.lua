-- ═══════════════════════════════════════════════════════════
-- 💬  DIALOGUE V3 — STEP 3: DIALOGUE UI  (E-key + Overhead Bubble)
-- Paste in:  View ▸ Command Bar   →   Enter
-- Creates:  StarterPlayerScripts ▸ DialogueUI  (LocalScript)
-- ═══════════════════════════════════════════════════════════
-- WHAT THIS DOES:
--   • Listens for the server's DialogueEvent (fires when you press E).
...
--   • Reads DialogueData (words) + ShopData (potions) → fully data-driven.
--   • Auto-closes if you walk too far away.
-- ═══════════════════════════════════════════════════════════
...
print("💬 DialogueUI V3 starting...")

-- ── safe module loader (no infinite yield — gives a CLEAR error) ──
local function loadModule(name)
	local m = ReplicatedStorage:FindFirstChild(name)
	if not m then m = ReplicatedStorage:WaitForChild(name, 15) end
	if not m then
		warn("❌❌ MISSING MODULE: " .. name .. "!")
		warn("   → Run DIALOGUE_V3_Step1_DataModules.lua in the Command Bar!")
		return nil
	end
	return require(m)
end

local DialogueData = loadModule("DialogueData")
local ShopData     = loadModule("ShopData")
if not DialogueData or not ShopData then
	warn("❌ DialogueUI V3 STOPPED — data modules are missing.")
	return
end
...
local function fmt(n)
	-- 🔢 Bulletproof 1K formatter (1000 → "1K"). Uses ShopData if present, else local copy.
	if ShopData and ShopData.Abbreviate then return ShopData.Abbreviate(n) end
	n = math.floor(tonumber(n) or 0)
	if n < 0 then return "-" .. fmt(-n) end
...
print("✅ Data loaded! NPC: " .. DialogueData.NPCName)

-- ═══════════════════════════════════════════════════════════
-- 💬 OVERHEAD BUBBLE (text above NPC head + bouncy pop)
-- ═══════════════════════════════════════════════════════════
...
local cScale = Instance.new("UIScale", container); cScale.Scale = 0
...
Instance.new("UIListLayout", optionsFrame).Padding = UDim.new(0, 6)

-- ═══════════════════════════════════════════════════════════
-- 🧠 LOGIC
-- ═══════════════════════════════════════════════════════════
local gotoNode, openShop, closeMenu, buyItem
local isTyping, typingThread = false, nil
local nodeHistory = {}
...
local function showLine(line) typeText(line); showBubble(line) end
...
local function makeButton(label, baseColor, order, onClick)
	...
	btn.MouseButton1Click:Connect(function()
		if isTyping then
			isTyping = false; bodyText.MaxVisibleGraphemes = -1; return
		end
		onClick()
	end)
	return btn
...
local function buildNodeOptions(node)
	...
	for i, opt in ipairs(node.Options or {}) do
		...
		makeButton(opt.Text or "?", color, i, function()
			if opt.Goto then
				table.insert(nodeHistory, currentNode)
				gotoNode(opt.Goto)
...
function buyItem(id, item)
	...
	task.spawn(function()
		local ok = PurchaseItemFunction:InvokeServer(id)
		if ok then
...
function openShop()
	pcall(function() ShopOpenEvent:FireServer() end)   -- server relay → ShopUI opens
...
-- ═══════════════════════════════════════════════════════════
-- 🎮 INPUT: server says "pressed E" → toggle dialogue
-- ═══════════════════════════════════════════════════════════
DialogueEvent.OnClientEvent:Connect(function()
	if container.Visible then closeMenu() else openMenu() end
end)

-- Escape also closes the dialogue (nice to have)
UserInputService.InputBegan:Connect(function(input, gpe)
	if gpe then return end
	if input.KeyCode == Enum.KeyCode.Escape and container.Visible then
		closeMenu()
	end
end)

-- ═══════════════════════════════════════════════════════════
-- 🔎 AUTO-SCAN + auto-close when walking away
-- ═══════════════════════════════════════════════════════════
local npcHead
RunService.Heartbeat:Connect(function()
	...
	-- auto-CLOSE if you walk too far (open is handled by pressing E)
	if container.Visible then
		local dist = (hrp.Position - npcHead.Position).Magnitude
		if dist > (DialogueData.TalkDistance or 12) + 8 then
			closeMenu()
		end
	end
end)

print("✅ DialogueUI V3 fully loaded! Press E near " .. DialogueData.NPCName .. " to talk.")
]====]

print("✅ STEP 3 done! DialogueUI (E-key) created in StarterPlayerScripts.")
