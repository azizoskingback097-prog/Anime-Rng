-- ═══════════════════════════════════════════════════════════
-- 💎 ROLL UI V6 (Free-Floating Glass Cards & Energy Beam)
-- ═══════════════════════════════════════════════════════════
-- Paste in Command Bar → Enter
--
-- UI REDESIGN:
--   • Removed the grey container. Cards float freely!
--   • Glassmorphism: Cards are transparent with glowing gradient borders.
--   • Text Hierarchy: Aura Name is huge, rarity is small/italic.
--   • Energy Beam: The selector line is now a glowing laser.
--   • Cleaned up buttons.
-- ═══════════════════════════════════════════════════════════
...
-- CONTAINER (Now transparent! No grey box!)
...
reelClip.BackgroundTransparency = 1 -- FULLY TRANSPARENT
reelClip.ClipsDescendants = true
reelClip.ZIndex = 5
reelClip.Parent = reelContainer

-- ENERGY BEAM (Replaces the boring line)
...
local function createCard(auraData, index)
	...
	-- GLASSMORPHISM: Slightly transparent dark background
	...
	grad.Color = ColorSequence.new({
		ColorSequenceKeypointoint.new(0, auraData.Color),
		ColorSequenceKeypoint.new(1, Color3.fromRGB(255, 255, 255))
	})
	grad.Rotation = 90
	grad.Parent = stroke
...
skipBtn.MouseButton1Click:Connect(function()
	if hasSkipGamepass then
		...
		if autoSkipEnabled then skipBtn.Text = "SKIP: ON"; skipBtn.BackgroundColor3 = Color3.fromRGB(0, 150, 0); skipBtn.TextColor3 = Color3.fromRGB(255,255,255)
		else skipBtn.Text = "SKIP"; skipBtn.BackgroundColor3 = Color3.fromRGB(40, 40, 40); skipBtn.TextColor3 = Color3.fromRGB(150,150,150) end
		if SFX then SFX.Play(gui, "click") end
	else
...
print("RollUI loaded! (V6 - Free-Float Redesign)")
]==]

print("✅ ROLL UI V6 REDESIGN APPLIED!")
