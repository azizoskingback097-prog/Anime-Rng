-- ═══════════════════════════════════════════════════════════
-- 🎓 VFX RIG SUPPORT — adds "Model" type for whole VFX rigs!
-- ═══════════════════════════════════════════════════════════
-- HOW TO USE: View > Command Bar → paste ALL of this → Enter
--
-- This adds a NEW VFX type called "Model" that can use your whole
-- VFX rigs (a Model with parts + particle emitters inside each part).
--
-- The Model type:
--   • Clones your ENTIRE rig
--   • Welds every part to your character (follows you!)
--   • Makes all parts invisible (you only see the particles)
--   • Turns ON every particle emitter inside
--   • Cleans up everything when you unequip
-- ═══════════════════════════════════════════════════════════
...
-- Make sure CustomVFX folder exists (where you put your rigs!)
ensure("Folder", "CustomVFX", RS)

-- ═══════════════════════════════════════════════════════════
-- VFX CLIENT — updated with the new "Model" type!
-- ═══════════════════════════════════════════════════════════
...
print("✨ VFXClient loaded (with Model/rig support!) — waiting for equip...")
...
-- ═══════════════════════════════════════════════════════════
-- 🏗️ SINGLE EFFECT CREATORS (Particle, Fire, Smoke, Light, Template)
-- ═══════════════════════════════════════════════════════════
...
-- Template = ONE single ParticleEmitter from CustomVFX folder
local function createTemplate(part, cfg)
	local template = CustomVFXFolder:FindFirstChild(cfg.TemplateName)
	if not template then
		warn("⚠️ VFX Template '" .. tostring(cfg.TemplateName) .. "' not found!")
		return nil
	end
...
-- ═══════════════════════════════════════════════════════════
-- 🌟 NEW! MODEL TYPE — uses your whole VFX rig!
-- ═══════════════════════════════════════════════════════════
-- This clones your ENTIRE rig (Model with parts + particles),
-- welds it to the character, makes parts invisible, and
-- turns on all particles inside it.
-- ═══════════════════════════════════════════════════════════
local function createModel(character, cfg)
	local template = CustomVFXFolder:FindFirstChild(cfg.TemplateName)
	if not template then
		warn("⚠️ VFX Model '" .. tostring(cfg.TemplateName) .. "' not found in CustomVFX folder!")
		print("   📁 Looking in: ReplicatedStorage > CustomVFX")
		print("   📋 Existing items in CustomVFX:")
	...
	print("🌟 Cloning VFX rig: " .. cfg.TemplateName .. " (" .. template.ClassName .. ")")
	local clone = template:Clone()

	-- find the body part to attach to
	local bodyPartName = cfg.Part or "HumanoidRootPart"
	...
	if not bodyPart then
		print("❌ Can't find body part: " .. bodyPartName)
		return nil
	end

	-- remember the original positions of all parts (so we can weld them correctly)
	local partOffsets = {}
	local firstPart = nil
	...
	for _, obj in ipairs(clone:GetDescendants()) do
		if obj:IsA("BasePart") then
			if not firstPart then
				firstPart = obj
				-- remember how far this part is from the model origin
				partOffsets[obj] = CFrame.new()
			else
				partOffsets[obj] = obj.CFrame
			end
		...
		if obj:IsA("ParticleEmitter") or obj:IsA("Fire") or obj:IsA("Smoke") then
			obj.Enabled = true
			print("   ✅ Enabled: " .. obj.ClassName .. " on " .. (obj.Parent and obj.Parent.Name or "?"))
		end
	...
	-- tag the clone itself too
	clone:SetAttribute(VFX_TAG, true)
	clone.Parent = character

	-- now weld everything to the body part
	-- we use a small delay so the parts have loaded their positions
	task.spawn(function()
		task.wait(0.1)

		if not clone.Parent or not bodyPart.Parent then return end

		-- if the rig has a PrimaryPart, use that as the main anchor
		local primaryPart = clone.PrimaryPart or firstPart

		if primaryPart then
			-- position the primary part at the body part
			primaryPart.CFrame = bodyPart.CFrame

			-- weld primary part to body part
			...
			-- weld all OTHER parts to the primary part (maintains rig shape)
			for _, obj in ipairs(clone:GetDescendants()) do
				if obj:IsA("BasePart") and obj ~= primaryPart then
					local weld = Instance.new("Weld")
					weld.Part0 = primaryPart
					weld.Part1 = obj
					weld.C0 = primaryPart.CFrame:Inverse() * obj.CFrame
					weld.Parent = obj
				end
			end
		else
			print("⚠️ No parts found in the rig — is it just a folder?")
		end
	end)
...
-- ═══════════════════════════════════════════════════════════
-- 🎬 APPLY VFX — creates all effects for a config
-- ═══════════════════════════════════════════════════════════
...
local function updateVFX()
	...
	if vfxConfig then
		...
		print("❌ No VFX found for '" .. baseName .. "' — check VFXData.AuraMap!")
	end
end
...
-- ═══════════════════════════════════════════════════════════
-- VFX DATA — updated with "Akaza" → "White" using Model type!
-- ═══════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════
-- 🌟 YOUR CUSTOM RIG VFX (uses the "Model" type!)
-- ═══════════════════════════════════════════════════════════
-- Type = "Model" means: clone the WHOLE rig from CustomVFX folder
-- TemplateName = the EXACT name of your rig in CustomVFX
-- Part = which body part to weld it to (HumanoidRootPart = body center)
-- ═══════════════════════════════════════════════════════════
VFXData.VFX["White Aura"] = {
	Name = "White Aura",
	Effects = {
		{
			Type = "Model",
...
function VFXData.GetVFXForAura(auraName, tier)
	if VFXData.AuraMap[auraName] then return VFXData.VFX[VFXData.AuraMap[auraName]] end
...
print("══════════════════════════════════════")
print("🌟 VFX RIG SUPPORT ADDED!")
print("══════════════════════════════════════")
print("✅ New 'Model' type: clones your whole VFX rig!")
print("✅ Akaza → White Aura (uses your rig from CustomVFX)")
print("══════════════════════════════════════")
print("🎮 HOW IT WORKS NOW:")
print("   • Your rig is cloned entirely (all parts + particles)")
print("   • Every part is welded to your character (follows you!)")
print("   • Parts become invisible (only particles show)")
print("   • All ParticleEmitters turn ON automatically")
print("   • Cleans up when you unequip")
print("══════════════════════════════════════")
print("📋 CHECKLIST before testing:")
print("   1. ReplicatedStorage > CustomVFX folder exists")
print("   2. Inside it: a Model named 'White' (your VFX rig)")
print("   3. The rig has parts with ParticleEmitters inside")
print("   4. Press Play → Admin → Give Akaza → Equip")
print("   5. Check Output for debug messages!")
print("══════════════════════════════════════")
